...
int out_index = 0;
int num_events = 0;
int num_items_in_buffer = 0; // this is "new items" not items in buffer
...
struct Node* LinkedList_addr = NULL;
...
//parses file for how many valid events are there to mmap properly
void count_events(void);

//~~~~Processes
void email_filter(void);
void calendar_filter(void);
...
{
  	//buf_size = atoi(argv[1]); we dont use this
  	
  	//we can quickly scan the file to see how many events are present in the input
	//get the number of accounts
	count_events();
	
  	
  	//This maps the "event list"
  	LinkedList_addr =  mmap(NULL,num_events*sizeof(struct Node), PROT_WRITE | PROT_READ, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
  	if(LinkedList_addr == MAP_FAILED)
  		exit(10);
  	
  	//create mmap region for the mutex and condition variable
  	mmap_mutex = mmap(NULL,sizeof(pthread_mutex_t), PROT_WRITE | PROT_READ, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
  	if(mmap_mutex == MAP_FAILED)
  		exit(11);
  		
  	mmap_cond_full = mmap(NULL,sizeof(pthread_cond_t), PROT_WRITE | PROT_READ, MAP_SHARED | MAP_ANONYMOUS, -1, 0);
  	if(mmap_cond_full == MAP_FAILED)
  		exit(12);
  		
  	pthread_mutex_init(&mmap_mutex[0],NULL);
  	pthread_cond_init(&mmap_cond_full[0],NULL);
  	//process 1 will parse the file and write to the memory mapped region (it will be redirected via stdio)
  	//process 2 will read from the memory mapped region and write to stdio
...
void email_filter(void)
{
    ...
    while(fgets(input_buffer,sizeof(input_buffer)+1,stdin) != NULL)
    {
        ...
        if(total_length_check && title_length_check && location_length_check && valid_command_check)
        {
            ...
            //lock thread
            pthread_mutex_lock(&mmap_mutex[0]);           
            
            
            
            //write to the buffer
            LinkedList_addr[in_index].command = data_to_copy[0];
            memcpy(LinkedList_addr[in_index].title,&data_to_copy[2],10);
            memcpy(LinkedList_addr[in_index].date,&data_to_copy[13],10);
            memcpy(LinkedList_addr[in_index].time,&data_to_copy[24],5);
            memcpy(LinkedList_addr[in_index].location,&data_to_copy[30],10);
      
            num_items_in_buffer++; // this keeps track of how many NEW entries in buffer
            in_index++;
            
    	    pthread_cond_signal(&mmap_cond_full[0]); // buffer now has a "new entry" to process
            

            pthread_mutex_unlock(&mmap_mutex[0]);
...
void calendar_filter(void)
{
    ...
    while(!done || num_items_in_buffer != 0) 
    {
    	...
    	pthread_mutex_lock(&mmap_mutex[0]); // i want to exclusively access this buffer
    	while(num_items_in_buffer == 0)
    		pthread_cond_wait(&mmap_cond_full[0],&mmap_mutex[0]); // wait until the email thread signals that its no longer empty
    	...
    	command = LinkedList_addr[out_index].command;
    	memcpy(title,LinkedList_addr[out_index].title,10);
    	memcpy(date,LinkedList_addr[out_index].date,10);
    	memcpy(time,LinkedList_addr[out_index].time,5);
    	memcpy(location,LinkedList_addr[out_index].location,10);
   	...
   	//If it got here, it took something out, so there is now a slot in the buffer
    	pthread_mutex_unlock(&mmap_mutex[0]); // done accessing buffer
...
void count_events(void)
{
	bool total_length_check,title_length_check,location_length_check,valid_command_check;
	//open the file
	FILE *ip_file_ptr = fopen("input.txt","r");
	char input_buffer[49]; //the input chars, process 100 emails, max is 49 [40 after you remove subject: ]
